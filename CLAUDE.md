...
- 🔒 **디자인은 절대 임의로 수정·변경하지 않는다.** 내용(문구·로직·데이터)이 바뀌어도 **기존 디자인은 그대로 유지**한다. **폰트·자간·행간·버튼 위치·박스 형태·가로/세로 폭 등 모든 디자인 요소**(`style`·레이아웃·간격·크기·색)의 변경이 필요하면 **먼저 사용자 승낙을 받은 후** 진행한다. 기능·문구만 바꾸고 디자인은 손대지 않는 것이 기본.
