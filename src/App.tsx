...
import {
  ...
  USAGE_SCOPES, USAGE_PERIODS, HOURS, MINS,
} from "./constants";
...
export default function App() {
  ...
  const resetProjectForm = () => {
    ...
    setPUsagePeriod(""); setPMemo(""); setPStatus("INQUIRY"); setPModelLines([]); setPModelSearch(""); setPMedia([]); setPRefImages([]); setPRefVideos([]);
  };
  ...
  const handleAddProject = async () => {
    ...
    for (let i=0; i<pModelLines.length; i++) {
      ...
      for (const b of conflicts) {
        const c = scheduleConflict(pStart, pEnd, b.start_time, b.end_time, pBookingType, b.booking_type);
  ...
  const handleSaveBookingEdit = async () => {
    ...
    for (const b of othersSameDay) {
      const c = scheduleConflict(selectedBooking.start_time, selectedBooking.end_time, b.start_time, b.end_time, selectedBooking.booking_type, b.booking_type);
      if (c.conflict) { conflict=true; reason=c.reason; break; }
    }
    if (conflict && selectedBooking.status!=="HOLD") updates.status = "HOLD";      // 새 충돌 → 자동 HOLD
    if (!conflict && selectedBooking.status==="HOLD") updates.status = "CHECKING"; // 충돌 해소 → HOLD 해제(스케줄확인)
    try {
      ...
      // ── 같은 모델의 다른 HOLD 건도 충돌이 풀렸으면 자동 해제 ──
      const affectedDates = [oldDate, selectedBooking.shoot_date].filter(Boolean);
      let releasedCnt = 0;
      for (const hb of updatedList.filter(b=>b.id!==selectedBooking.id&&b.model_id===selectedBooking.model_id&&b.status==="HOLD"&&affectedDates.includes(b.shoot_date))) {
  ...
  const filteredBookings  = bookings.filter(b=>{
    ...
    return matchQ&&matchSt&&matchMg&&matchMo;
  });
      ...
      {selectedBooking&&(
        <Modal onClose={()=>{ setSelectedBooking(null); setEditingBooking(false); }} wide>
          ...
          <div style={{ marginBottom:16 }}>
            {!editingBooking
              ? <Badge code={selectedBooking.status} />
              : (
                <>
          ...
          {!editingBooking ? (
            <>
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:12, marginBottom:14 }}>
                {/* 모델 (클릭 → 모델 상세) */}
                <div>
                  <p style={{ margin:0, fontSize:11, color:C.muted }}>모델</p>
                  {(()=>{ const m=models.find(m=>m.id===selectedBooking.model_id); return m
                    ? <p onClick={()=>{ setSelectedBooking(null); setEditingBooking(false); setModelHistAll(false); setMEditMode(false); setSelectedModel(m); }} style={{ margin:"3px 0 0", fontSize:13, fontWeight:700, color:C.blue, cursor:"pointer", textDecoration:"underline" }}>{m.name} →</p>
                    : <p style={{ margin:"3px 0 0", fontSize:13, fontWeight:600, color:C.text }}>-</p>; })()}
                </div>
                {/* 고객사 (클릭 → 고객사 상세) */}
                <div>
                  <p style={{ margin:0, fontSize:11, color:C.muted }}>고객사</p>
                  {(()=>{ const c=customers.find(c=>c.id===selectedBooking.customer_id); return c
                    ? <p onClick={()=>{ setSelectedBooking(null); setEditingBooking(false); setCEditMode(false); setSelectedCustomer(c); }} style={{ margin:"3px 0 0", fontSize:13, fontWeight:700, color:C.blue, cursor:"pointer", textDecoration:"underline" }}>{c.name} →</p>
                    : <p style={{ margin:"3px 0 0", fontSize:13, fontWeight:600, color:C.text }}>-</p>; })()}
                </div>
                {[
            ...
            <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:10, marginBottom:10 }}>
                <div><label style={{ fontSize:11, color:C.muted, display:"block", marginBottom:5 }}>담당자</label>
              ...
              <div style={{ marginTop:10, marginBottom:10 }}>
                <label style={{ fontSize:11, color:C.muted, display:"block", marginBottom:6 }}><Camera size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> 촬영 레퍼런스 (이미지 8장 · 영상 링크 2개)</label>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:6 }}>
                ...
                {(selectedBooking.reference_videos||[]).length<2&&(
                  <button type="button" onClick={()=>{ const u=promptVideoUrl(); if(u) setSelectedBooking((pp:any)=>({ ...pp, reference_videos:[...(pp.reference_videos||[]), u] })); }} style={{ background:"transparent", border:`1px dashed ${C.border}`, borderRadius:14, padding:"3px 10px", fontSize:11, color:C.muted, cursor:"pointer" }}>+ 영상 링크</button>
                )}
              </div>
              </div>
            </>
        ...
        return (
          <Modal onClose={()=>setSelectedProjectId(null)} wide>
            ...
            <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr 1fr", gap:12, marginBottom:14 }}>
              <div><p style={{ margin:0, fontSize:11, color:C.muted }}>촬영일</p><p style={{ margin:"3px 0 0", fontSize:13, fontWeight:700, color:C.text }}>{fmtDate(pbs[0]?.shoot_date)}</p></div>
              <div><p style={{ margin:0, fontSize:11, color:C.muted }}>모델</p><p style={{ margin:"3px 0 0", fontSize:13, fontWeight:700, color:C.text }}>{pbs.length}명</p></div>
              <div><p style={{ margin:0, fontSize:11, color:C.muted }}>총 금액</p><p style={{ margin:"3px 0 0", fontSize:13, fontWeight:800, color:C.yellow }}>{totalFee>0?totalFee.toLocaleString()+"원":"-"}</p></div>
            </div>
