...
export default function TimePicker({ label, value, onChange }: { label:string; value:string; onChange:(v:string)=>void }) {
  ...
  const sel: CSSProperties = { background:"var(--c-card2)", border:`1px solid ${C.border}`, borderRadius:5, color:C.text, fontSize:12, padding:"4px 3px", cursor:"pointer" };
  const ap = (val: string): CSSProperties => ({ padding:"4px 7px", border:`1px solid ${val===ampm?C.blue:C.border}`, borderRadius:5, cursor:"pointer", fontSize:11, fontWeight:700, background:val===ampm?C.blue+"22":"var(--c-card2)", color:val===ampm?C.blue:C.muted });
