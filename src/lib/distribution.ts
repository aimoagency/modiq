...
export const sendDistribution = async (opts: {
  ...
  if (modelRows.length) {
    try { await sb("distribution_models", "POST", modelRows); }
      ...
      if (/is_foreigner|visa_entry|visa_exit|category|career_years|country|instagram_url|PGRST204|schema cache/i.test(msg)) {
        const stripped = modelRows.map(({ is_foreigner, visa_entry, visa_exit, category, career_years, country, instagram_url, ...r }) => r);
...
export const loadSharedSchedule = async (sourceModelIds: (string | null | undefined)[]): Promise<Record<string, SharedBusy[]>> => {
  ...
  if (!ids.length) return {};
  try {
    const rows: SharedBusy[] = await sb("rpc/shared_model_schedule", "POST", { p_model_ids: ids });
    const map: Record<string, SharedBusy[]> = {};
    (rows || []).forEach(r => { (map[r.model_id] ||= []).push(r); });
    return map;
  } catch { return {}; }
};
