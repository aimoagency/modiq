  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <h1 style={{ margin:0, fontSize:24, fontWeight:800, color:C.text }}><ClipboardList size={20} style={{ verticalAlign:-2, flexShrink:0 }}/> 섭외 ({filteredBookings.length}건)</h1>
      ...
      {filteredBookings.length===0 ? <p style={{ color:C.muted }}>결과 없음</p> : (()=>{
        ...
        const Card=(b:any)=> isMobile ? (
            <div key={b.id} onClick={()=>setSelectedBooking(b)} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 14px", cursor:"pointer" }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5 }}>
                ...
                {(()=>{ const m=models.find((mm:any)=>mm.id===b.model_id); return m?.thumb_url
                  ...
                  : <span style={{ width:24, height:24, borderRadius:"50%", background:"linear-gradient(135deg,#c9a96e,#8b6a3e)", display:"inline-flex", alignItems:"center", justifyContent:"center", color:"white", fontSize:11, fontWeight:800, flexShrink:0 }}>{(m?.name||"?")[0]}</span>; })()}
                <strong style={{ flex:1, fontSize:15, fontWeight:700, color:C.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{models.find((m:any)=>m.id===b.model_id)?.name||"?"} → {customers.find((c:any)=>c.id===b.customer_id)?.name||"?"}</strong>
                <Badge code={b.status} type={b.booking_type} />
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:14, color:C.textSub }}>
            ...
            >
              ...
              <p style={{ flex:1, margin:0, fontSize:14, color:C.muted, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                <strong style={{ fontSize:15, fontWeight:700, color:C.text }}>{models.find((m:any)=>m.id===b.model_id)?.name||"?"} → {customers.find((c:any)=>c.id===b.customer_id)?.name||"?"}</strong>
              ...
              {b.shoot_fee?<span style={{ color:C.yellow, fontWeight:700, fontSize:14, flexShrink:0, marginRight:4 }}><Coins size={12} style={{ verticalAlign:-2, flexShrink:0 }}/>{b.shoot_fee.toLocaleString()}원</span>:null}
              <Badge code={b.status} type={b.booking_type} />
            </div>
            );
        return (
              ...
              return (
                <div key={"g"+oi} style={{ border:`1px solid ${C.blue}55`, borderRadius:12, overflow:"hidden" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 14px", background:C.blue+"14", borderBottom:`1px solid ${C.blue}33`, flexWrap:"wrap" }}>
                    ...
                    <span style={{ fontSize:15, fontWeight:700, color:C.text }}>{bs[0].project_name||"프로젝트"} <span style={{ color:C.muted, fontWeight:400 }}>· {customers.find((c:any)=>c.id===bs[0].customer_id)?.name||"?"}</span></span>
                    ...
                    <span style={{ fontSize:13, color:C.muted }}>모델 {bs.length}명</span>
                    <span style={{ fontSize:13, color:C.muted }}>· {fmtDate(bs[0].shoot_date)}</span>
                    {total>0&&<span style={{ marginLeft:"auto", fontSize:15, color:C.yellow, fontWeight:800 }}>{total.toLocaleString()}원</span>}
                  </div>
