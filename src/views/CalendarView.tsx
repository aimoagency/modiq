...
import { visaDday, fmtTime } from "../lib/utils";
import Badge from "../components/Badge";
import Modal from "../components/Modal";
import TypeIcon from "../components/TypeIcon";
import { User, CalendarDays, CalendarOff, ClipboardList, Clock, MapPin, Folder, Plane, AlertTriangle, Flag } from "../components/icons";
                ...
                {isMobile ? (
                  dayBookings.length>0 ? (
                    <div style={{ display:"flex", gap:3, justifyContent:"center" }}>
                      {dayBookings.slice(0,3).map((b,bi)=>{ const ds=STATUS[b.status]||STATUS.INQUIRY; return <span key={bi} style={{ width:6, height:6, borderRadius:"50%", background:ds.color, display:"inline-block" }}/>; })}
                      {dayBookings.length>3&&<span style={{ fontSize:8, color:C.muted, lineHeight:"6px" }}>+</span>}
                    </div>
                  ) : null
                ) : (
                ...
                {[...selDateBookings].sort((a,b)=>(a.start_time||"") < (b.start_time||"") ? -1:1).map((b,idx)=>{
                  ...
                  if (isMobile) return (
                    <div key={b.id} onClick={()=>onSelectBooking(b)}
                      style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"12px 14px", background:C.card2, borderRadius:10, border:`1px solid ${C.border}` }}>
                      <span style={{ width:20, height:20, borderRadius:"50%", background:s.color+"33", color:s.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:800, flexShrink:0, marginTop:2 }}>{idx+1}</span>
                      {model?.thumb_url
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <p style={{ margin:0, fontSize:13, fontWeight:700, color:C.text, lineHeight:1.3 }}>{model?.name||"?"} <span style={{ color:C.muted, fontWeight:400, fontSize:12 }}>→ {client?.name||"?"}</span></p>
                        <div style={{ display:"flex", flexWrap:"wrap", gap:"3px 8px", marginTop:4, fontSize:11, color:C.muted }}>
                          ...
                          {b.project_name&&<span><Folder size={10} style={{ verticalAlign:-1.5, flexShrink:0 }}/> {b.project_name}</span>}
                        </div>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6, flexShrink:0 }}>
                        ...
                        <Badge code={b.status} />
                        {b.status==="CONFIRMED"&&!b.deposit_paid&&<span style={{ fontSize:10, color:C.red, fontWeight:700 }}>계약금 미입금</span>}
                      </div>
                    </div>
                  );
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:3, flexWrap:"wrap" }}>
                          ...
                          <span style={{ background:bt.color+"22", color:bt.color, border:`1px solid ${bt.color}44`, borderRadius:4, padding:"1px 7px", fontSize:10, fontWeight:700 }}><TypeIcon type={b.booking_type} size={10}/> {bt.label}</span>
                        </div>
                        <div style={{ fontSize:11, color:C.muted, display:"flex", gap:8, flexWrap:"wrap" }}>
                          {b.start_time&&<span><Clock size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> {fmtTime(b.start_time,b.end_time)}</span>}
                          {b.location&&<span><MapPin size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> {b.location}</span>}
                          {b.project_name&&<span><Folder size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> {b.project_name}</span>}
                        </div>
                      </div>
                      <div style={{ textAlign:"right", flexShrink:0 }}>
                        {b.shoot_fee>0&&<p style={{ margin:0, fontSize:13, color:"#c9a96e", fontWeight:800 }}>{b.shoot_fee.toLocaleString()}원</p>}
                        {b.status==="CONFIRMED"&&!b.deposit_amt&&<p style={{ margin:"3px 0 0", fontSize:10, color:C.red, fontWeight:700 }}>계약금 미설정</p>}
                      </div>
                      <Badge code={b.status} />
