  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <h1 style={{ margin:0, fontSize:24, fontWeight:800, color:C.text }}><CalendarDays size={20} style={{ verticalAlign:-2, flexShrink:0 }}/> 캘린더</h1>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14, flexWrap:"wrap" }}>
        <span style={{ fontSize:13, color:C.muted, fontWeight:600 }}>모델 필터:</span>
        <button onClick={()=>setModelFilter("")} style={{ padding:"4px 12px", borderRadius:20, border:`1px solid ${!modelFilter?C.blue:C.border}`, background:!modelFilter?C.blue+"22":"transparent", color:!modelFilter?C.blue:C.muted, fontSize:13, fontWeight:600, cursor:"pointer" }}>전체</button>
        {models.map(m=>{
          const isSel = modelFilter===m.id;
          return (
            <button key={m.id} onClick={()=>{ setModelFilter(isSel?"":m.id); setSelDate(null); }}
              style={{ display:"flex", alignItems:"center", gap:5, padding:"4px 10px", borderRadius:20, border:`1px solid ${isSel?C.blue:C.border}`, background:isSel?C.blue+"22":"transparent", color:isSel?C.blue:C.textSub, fontSize:13, fontWeight:isSel?700:400, cursor:"pointer" }}
            >
              {m.thumb_url
                ? <img src={m.thumb_url} alt="" style={{ width:16, height:16, borderRadius:"50%", objectFit:"cover" }} />
                : <span style={{ width:16, height:16, borderRadius:"50%", background:"linear-gradient(135deg,#c9a96e,#8b6a3e)", display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:10, color:"white", fontWeight:800 }}>{m.name[0]}</span>
              }
              {m.name}
              {m.is_foreigner&&<Plane size={10} style={{ flexShrink:0 }}/>}
            </button>
          );
        })}
      </div>
      ...
      {filteredModel?.is_foreigner&&visaEntry&&visaExit&&(
        <div style={{ display:"flex", gap:12, alignItems:"center", background:"#1a2f4a", border:`1px solid ${C.blue}50`, borderRadius:8, padding:"8px 14px", marginBottom:14 }}>
          ...
          <span style={{ fontSize:13, color:C.textSub }}><strong style={{ color:C.text }}>{filteredModel.name}</strong> 비자 유효 기간</span>
          <span style={{ fontSize:13, color:C.blue, fontWeight:700 }}>{visaEntry.replace(/-/g,".")} ~ {visaExit.replace(/-/g,".")}</span>
          {(()=>{
            const dday = visaDday(visaExit);
            const color = dday==="만료"?C.red:parseInt(dday.slice(2))<=7?C.orange:C.green;
            return <span style={{ marginLeft:"auto", background:color+"22", color, border:`1px solid ${color}44`, borderRadius:20, padding:"2px 10px", fontSize:12, fontWeight:700 }}>{dday}</span>;
          })()}
        </div>
      )}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16, flexWrap:"wrap" }}>
        <button onClick={prevMonth} style={{ background:C.card, border:`1px solid ${C.border}`, color:C.text, borderRadius:8, padding:"6px 13px", cursor:"pointer", fontSize:18 }}>‹</button>
        <h2 style={{ margin:0, fontSize:22, fontWeight:800, color:C.text, minWidth:140, textAlign:"center" }}>{calYear}년 {calMonth+1}월</h2>
        <button onClick={nextMonth} style={{ background:C.card, border:`1px solid ${C.border}`, color:C.text, borderRadius:8, padding:"6px 13px", cursor:"pointer", fontSize:18 }}>›</button>
        <button onClick={()=>{ setCalYear(today.getFullYear()); setCalMonth(today.getMonth()); setSelDate(todayStr); }} style={{ ...btnS(C.blue), padding:"5px 14px", fontSize:13 }}>오늘</button>
        {(()=>{ const cnt=filteredBookings.filter(b=>b.shoot_date?.startsWith(monthStr)).length; return cnt>0?<span style={{ fontSize:13, color:C.muted }}>이달 <strong style={{ color:C.text }}>{cnt}건</strong></span>:null; })()}
        {/* [추가] 이달 충돌 건수 요약 */}
        {(()=>{
          ...
          return <span style={{ display:"inline-flex", alignItems:"center", gap:4, background:col+"18", border:`1px solid ${col}44`, borderRadius:20, padding:"3px 11px", fontSize:13, color:col, fontWeight:700 }}>
        ...
        return (
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 16px", borderBottom:`1px solid ${C.border}`, flexWrap:"wrap" }}>
              <button onClick={()=>setSelDate(null)} style={{ background:C.card2, border:`1px solid ${C.border}`, color:C.text, borderRadius:8, padding:"5px 11px", cursor:"pointer", fontSize:14 }}>‹ 달력</button>
              <span style={{ fontSize:18, fontWeight:800, color:C.text }}><ClipboardList size={15} style={{ verticalAlign:-2, flexShrink:0 }}/> {selDate!.replace(/-/g,".")}</span>
              <span style={{ fontSize:13, color:C.muted }}>총 <strong style={{ color:C.text }}>{selDateBookings.length}건</strong> · 모델 {dayModels.length}명</span>
              {dayConflict&&<span style={{ display:"inline-flex", alignItems:"center", gap:4, background:cWorst+"18", border:`1px solid ${cWorst}44`, borderRadius:20, padding:"3px 10px", fontSize:12, color:cWorst, fontWeight:700 }}><AlertTriangle size={11} style={{ flexShrink:0 }}/> 더블부킹 감지</span>}
              <div style={{ marginLeft:"auto", display:"flex", gap:0, background:C.card2, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
                {(["list","timeline"] as const).map(v=>(
                  <button key={v} onClick={()=>setDayView(v)} style={{ padding:"6px 14px", borderRadius:6, border:"none", background:dayView===v?C.blue:"transparent", color:dayView===v?"white":C.muted, fontSize:13, fontWeight:700, cursor:"pointer" }}>{v==="list"?"목록 보기":"타임라인"}</button>
                ))}
              </div>
              <button onClick={()=>onAddBooking(modelFilter||undefined, selDate||undefined)} style={{ ...btnS(C.green), padding:"6px 14px", fontSize:13 }}>+ 섭외 추가</button>
            </div>

            {selDateBookings.length===0 ? (
              <div style={{ padding:"48px 0", textAlign:"center", color:C.muted, fontSize:14 }}>이 날 섭외 일정이 없습니다.</div>
            ) : (
            <div style={{ display:"flex", overflowX:"auto" }}>
              ...
              <div style={{ width:56, flexShrink:0, borderRight:`1px solid ${C.border}` }}>
                ...
                <div style={{ position:"relative", height:gridH }}>
                  {hours.map((h,i)=>(<div key={h} style={{ position:"absolute", top:i*PXH-7, right:8, fontSize:12, color:C.muted, fontWeight:600 }}>{String(h).padStart(2,"0")}:00</div>))}
                </div>
              </div>
              {/* 모델 컬럼 */}
              {dayModels.map((m:any)=>{
                  ...
                  <div key={m.id} style={{ flex:"1 0 200px", minWidth:200, borderRight:`1px solid ${C.border}` }}>
                    <div style={{ height:40, display:"flex", alignItems:"center", gap:8, padding:"0 12px", borderBottom:`1px solid ${C.border}`, background:C.card2 }}>
                      {m.thumb_url
                        ...
                        : <span style={{ width:26, height:26, borderRadius:"50%", background:"linear-gradient(135deg,#c9a96e,#8b6a3e)", display:"inline-flex", alignItems:"center", justifyContent:"center", color:"white", fontSize:12, fontWeight:800, flexShrink:0 }}>{(m.name||"?")[0]}</span>}
                      <span style={{ fontSize:14, fontWeight:700, color:C.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{m.name}</span>
                      {m.is_foreigner&&m.visa_exit&&(()=>{ const dd=visaDday(m.visa_exit); const col=dd==="만료"?C.red:parseInt(dd.slice(2))<=7?C.orange:C.green; return <span style={{ marginLeft:"auto", display:"inline-flex", alignItems:"center", gap:2, color:col, fontSize:11, fontWeight:700, flexShrink:0 }}><Plane size={9} style={{ flexShrink:0 }}/>{dd}</span>; })()}
                    </div>
                    <div style={{ position:"relative", height:gridH }}>
                      ...
                      {mbs.map(b=>{
                        ...
                        return (
                          <div key={b.id} onClick={()=>onSelectBooking(b)} title={`${m.name} · ${client?.name||"?"} · ${bt.label} · ${st.label}`}
                            ...
                            <div style={{ fontSize:12, fontWeight:800, color:st.color, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{fmtTime(b.start_time,b.end_time)}</div>
                            <div style={{ fontSize:13, fontWeight:700, color:C.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{client?.name||"?"}</div>
                            {bh>54&&<div style={{ fontSize:11, color:bt.color, fontWeight:700, marginTop:1, whiteSpace:"nowrap" }}><TypeIcon type={b.booking_type} size={9}/> {bt.label}</div>}
                            {isC&&<div style={{ marginTop:2, display:"inline-flex", alignItems:"center", gap:3, color:cWorst, fontSize:10, fontWeight:800 }}><AlertTriangle size={9} style={{ flexShrink:0 }}/> 시간 겹침</div>}
                          </div>
                        );
                      })}
                    </div>
            ...
            {selDateBookings.length>0&&(
              <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 16px", borderTop:`1px solid ${C.border}`, background:C.card2, flexWrap:"wrap" }}>
                ...
                <span style={{ fontSize:14, fontWeight:700, color:C.text }}>{selDate!.slice(5).replace("-",".")} 정산 요약</span>
                <span style={{ fontSize:13, color:C.muted }}>섭외 {selDateBookings.length}건</span>
                {holdCnt>0&&<span style={{ fontSize:13, color:C.yellow, fontWeight:700 }}><AlertTriangle size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> HOLD {holdCnt}건</span>}
                <span style={{ marginLeft:"auto", fontSize:20, fontWeight:800, color:C.yellow }}>{dayTotal.toLocaleString()}원</span>
              </div>
            )}
          </div>
        );
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden", ...((!isMobile && selDate)?{flex:1, minWidth:0}:{marginBottom:selDate?16:0}) }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", borderBottom:`1px solid ${C.border}` }}>
          {DOW.map((d,i)=>(
            <div key={d} style={{ padding:"9px 0", textAlign:"center", fontSize:12, fontWeight:700, color:i===0?C.red:i===6?C.blue:C.muted }}>{d}</div>
          ))}
        </div>
        {/* 날짜 셀 */}
            ...
            return (
              ...
              >
                ...
                <div style={{ marginBottom:3, textAlign:isMobile?"center":"left" }}>
                  <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:22, height:22, borderRadius:"50%", fontSize:13, fontWeight:isToday?800:600, background:isToday?C.blue:isVisaExit?C.red+"22":"transparent", color:isToday?"white":isVisaExit?C.red:!cell.cur?C.border:(krHol||manHol)?C.red:col===0?C.red:col===6?C.blue:C.text }}>{cell.day}</span>
                  {/* [추가] 충돌 ⚠️ 배지 */}
                  ...
                  {!isMobile&&isVisaExit&&<span style={{ fontSize:10, color:C.red, fontWeight:700, marginLeft:2 }}>출국</span>}
                  {!isMobile&&krHol&&<span style={{ fontSize:10, color:C.red, fontWeight:700, marginLeft:2 }}>{krHol}</span>}
                  {!isMobile&&!krHol&&manHol&&<span style={{ fontSize:10, color:C.orange, fontWeight:700, marginLeft:2 }}>{manHol.label}</span>}
                </div>
                ...
                {isMobile ? (
                  dayBookings.length>0 ? (
                    <div style={{ display:"flex", gap:3, justifyContent:"center" }}>
                      {dayBookings.slice(0,3).map((b,bi)=>{ const ds=STATUS[b.status]||STATUS.INQUIRY; const isC=dayConflict?.conflictIds.has(b.id); return <span key={bi} style={{ width:6, height:6, borderRadius:"50%", background:isC?conflictColor!:ds.color, display:"inline-block", ...(isC?{boxShadow:`0 0 0 1.5px ${conflictColor}`}:{}) }}/>; })}
                      {dayBookings.length>3&&<span style={{ fontSize:9, color:C.muted, lineHeight:"6px" }}>+</span>}
                    </div>
                  ) : null
                ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
                  {dayBookings.slice(0,2).map((b,bi)=>{
                    ...
                    return (
                      <div key={bi} onClick={e=>{ e.stopPropagation(); onSelectBooking(b); }}
                        style={{ background:s.color+"28", color:s.color, border:`1px solid ${isConflict?(conflictColor!):s.color+"44"}`, borderRadius:4, padding:"2px 5px", fontSize:11, fontWeight:700, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", cursor:"pointer", display:"flex", alignItems:"center", gap:3 }}
                        title={`${m?.name||"?"} · ${bt.label} · ${s.label}${isConflict?" · ⚠️ 일정 충돌":""}`}
                      >
                        {isConflict&&<AlertTriangle size={9} color={conflictColor!} strokeWidth={2.6} style={{ flexShrink:0 }}/>}
                        <TypeIcon type={b.booking_type} size={9}/>
                        {m?.thumb_url
                          ? <img src={m.thumb_url} alt="" style={{ width:11, height:11, borderRadius:"50%", objectFit:"cover", flexShrink:0 }} />
                          : <span style={{ fontSize:10 }}>{(m?.name||"?")[0]}</span>
                        }
                  ...
                  {dayBookings.length>2&&(
                    <div onClick={e=>{ e.stopPropagation(); cell.date&&setSelDate(cell.date); }}
                      style={{ fontSize:11, color:C.blue, fontWeight:700, padding:"2px 5px", background:C.blue+"18", borderRadius:4, cursor:"pointer", textAlign:"center" }}>
      ...
      {selDate&&(
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:16, ...(!isMobile?{width:380, flexShrink:0, maxHeight:"calc(100vh - 170px)", overflowY:"auto"}:{}) }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            <div>
              <p style={{ margin:0, fontWeight:800, fontSize:17, color:C.text }}><ClipboardList size={14} style={{ verticalAlign:-2, flexShrink:0 }}/> {selDate.replace(/-/g,".")}</p>
              <p style={{ margin:"3px 0 0", fontSize:13, color:C.muted }}>
                ...
                {isOutsideVisa(selDate)&&filteredModel&&<span style={{ marginLeft:8, color:C.red, fontWeight:700, fontSize:12 }}><AlertTriangle size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> 비자 범위 밖</span>}
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              {holidayByDate[selDate]
                ? <button onClick={()=>onDeleteHoliday&&onDeleteHoliday(holidayByDate[selDate].id)} style={{ ...btnS("#3a1a1a"), border:`1px solid ${C.red}55`, color:C.red, padding:"6px 14px", fontSize:13 }}><CalendarOff size={13} style={{ verticalAlign:-2, flexShrink:0 }}/> 휴무일 해제</button>
                : <button onClick={()=>{ setHDate(selDate); setHLabel("휴무일"); setShowHolidayForm(true); }} style={{ ...btnS(C.card2), border:`1px solid ${C.border}`, color:C.textSub, padding:"6px 14px", fontSize:13 }}><CalendarOff size={13} style={{ verticalAlign:-2, flexShrink:0 }}/> 휴무일 지정</button>
              }
              {/* 날짜 클릭 → 해당 모델 + 날짜 pre-선택 섭외 등록 */}
              <button onClick={()=>onAddBooking(modelFilter||undefined, selDate)}
                style={{ ...btnS(C.green), padding:"6px 14px", fontSize:13 }}>
          ...
          {!isMobile&&(
            <div style={{ display:"flex", gap:0, marginBottom:12, background:C.card2, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
              {(["list","timeline"] as const).map(v=>(
                <button key={v} onClick={()=>setDayView(v)} style={{ flex:1, padding:"6px 0", borderRadius:6, border:"none", background:dayView===v?C.blue:"transparent", color:dayView===v?"white":C.muted, fontSize:13, fontWeight:700, cursor:"pointer" }}>{v==="list"?"목록 보기":"타임라인"}</button>
              ))}
            </div>
          )}
          ...
          {conflictByDate[selDate]&&(()=>{
            ...
            return (
              <div style={{ display:"flex", alignItems:"flex-start", gap:8, background:col+"18", border:`1px solid ${col}44`, borderRadius:8, padding:"10px 12px", marginBottom:12 }}>
                ...
                <div>
                  <p style={{ margin:0, fontSize:13, color:col, fontWeight:700 }}>
                    {cInfo.worst==="OVERLAP" ? "일정 충돌 — 시간 겹침" : "일정 충돌 — 완충시간 부족"}
                  </p>
                  <p style={{ margin:"3px 0 0", fontSize:12, color:C.textSub, lineHeight:1.5 }}>
                    {cInfo.worst==="OVERLAP"
          ...
          {selDateBookings.length===0
            ? (
              <div style={{ textAlign:"center", padding:"24px 0" }}>
                <p style={{ color:C.muted, fontSize:14, margin:"0 0 12px" }}>이 날 섭외 일정이 없습니다.</p>
                {isOutsideVisa(selDate)&&filteredModel&&<p style={{ color:C.red, fontSize:13, margin:0 }}><AlertTriangle size={11} style={{ verticalAlign:-2, flexShrink:0 }}/> {filteredModel.name}의 비자 유효 기간({visaEntry?.replace(/-/g,".")} ~ {visaExit?.replace(/-/g,".")}) 밖입니다.</p>}
              </div>
            )
            : (()=>{
                const sorted=[...selDateBookings].sort((a,b)=>(a.start_time||"")<(b.start_time||"")?-1:1);
                ...
                const Card=(b:any)=>{ n+=1; const idx=n;
                  ...
                  return (
                    <div key={b.id} onClick={()=>onSelectBooking(b)}
                      ...
                      <span style={{ width:20, height:20, borderRadius:"50%", background:s.color+"33", color:s.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:800, flexShrink:0, marginTop:2 }}>{idx}</span>
                      {model?.thumb_url
                        ...
                        : <div style={{ width:34, height:34, borderRadius:"50%", background:"linear-gradient(135deg,#c9a96e,#8b6a3e)", display:"flex", alignItems:"center", justifyContent:"center", color:"white", fontWeight:800, fontSize:14, flexShrink:0 }}>{(model?.name||"?")[0]}</div>
                      }
                      <div style={{ flex:1, minWidth:0 }}>
                        <p style={{ margin:0, fontSize:15, fontWeight:700, color:C.text, lineHeight:1.3 }}>
                          {model?.name||"?"} <span style={{ color:C.muted, fontWeight:400, fontSize:14 }}>→ {client?.name||"?"}</span>
                        ...
                        <div style={{ display:"flex", flexWrap:"wrap", gap:"3px 8px", marginTop:4, fontSize:14, color:C.muted }}>
                      ...
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6, flexShrink:0 }}>
                        {b.shoot_fee>0&&<span style={{ fontSize:14, color:C.yellow, fontWeight:800, whiteSpace:"nowrap" }}>{b.shoot_fee.toLocaleString()}원</span>}
                        <Badge code={b.status} type={b.booking_type} />
                        {b.status==="CONFIRMED"&&!b.deposit_amt&&<span style={{ fontSize:11, color:C.red, fontWeight:700 }}>계약금 미설정</span>}
                      </div>
                    </div>
                  );
                };
                      ...
                      return (
                        <div key={pid} style={{ border:`1px solid ${C.blue}55`, borderRadius:12, overflow:"hidden" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 12px", background:C.blue+"14", borderBottom:`1px solid ${C.blue}33` }}>
                            ...
                            <span style={{ fontSize:14, fontWeight:700, color:C.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{bs[0].project_name||"프로젝트"} <span style={{ color:C.muted, fontWeight:400 }}>· {customers.find(c=>c.id===bs[0].customer_id)?.name||"?"}</span></span>
                            <div style={{ display:"flex", marginLeft:6 }}>
                              {ms.slice(0,3).map((m:any,i)=>(m.thumb_url
                                ...
                                : <span key={i} style={{ width:20, height:20, borderRadius:"50%", background:"linear-gradient(135deg,#c9a96e,#8b6a3e)", border:`2px solid ${C.card}`, marginLeft:i?-7:0, display:"inline-flex", alignItems:"center", justifyContent:"center", color:"white", fontSize:10, fontWeight:800 }}>{(m.name||"?")[0]}</span>
                              ))}
                            </div>
                            <span style={{ fontSize:12, color:C.muted, whiteSpace:"nowrap" }}>모델 {bs.length}명</span>
                            {total>0&&<span style={{ marginLeft:"auto", fontSize:14, color:C.yellow, fontWeight:800, whiteSpace:"nowrap" }}>{total.toLocaleString()}원</span>}
                          </div>
      ...
      {showHolidayForm&&(
        <Modal onClose={()=>setShowHolidayForm(false)}>
          ...
          <label style={{ fontSize:12, color:C.muted, display:"block", marginBottom:5 }}>날짜 *</label>
          <input style={inp} type="date" value={hDate} onChange={e=>setHDate(e.target.value)} />
          <label style={{ fontSize:12, color:C.muted, display:"block", marginBottom:5 }}>휴무일 내용 *</label>
          ...
          <div style={{ display:"flex", gap:10, marginTop:6 }}>
            ...
            }} style={{ ...btnS(C.green), flex:1, padding:"10px 0", fontSize:14 }}>저장</button>
            <button onClick={()=>setShowHolidayForm(false)} style={{ ...btnS("#333"), flex:1, padding:"10px 0", fontSize:14 }}>취소</button>
          </div>
        </Modal>
      )}
    </div>
