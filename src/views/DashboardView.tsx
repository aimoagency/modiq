  ...
  <div>
        ...
        <div style={{ background:C.card, border:`1px solid ${overdue>0?C.red:C.yellow}55`, borderRadius:10, padding:"16px 18px", marginBottom:16 }}>
          ...
          <div>
            ...
            return items.map((it,idx)=>{
              ...
              // 모바일: 가로폭 초과 방지 — 2줄 세로 스택(상단 D-day·고객사·모델·금액 / 하단 날짜·계약금잔금)
              if (isMobile) return (
                <div key={it.b.id+it.label+idx} onClick={()=>setSelectedBooking(it.b)} style={{ borderTop:top(), padding:"10px 4px", cursor:"pointer" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                    <span style={{ fontSize:11, fontWeight:800, color, background:color+"1a", borderRadius:6, padding:"3px 8px", flexShrink:0, whiteSpace:"nowrap" }}>{ddayLabel}</span>
                    <span style={{ flex:1, minWidth:0, fontSize:13, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}><strong style={{ fontWeight:700, color:C.text }}>{client?.name||"?"}</strong><span style={{ color:C.muted, fontWeight:600 }}> · {model?.name||"?"}</span></span>
                    <span style={{ fontSize:13, fontWeight:800, color:it.label==="잔금"?C.text:C.textSub, whiteSpace:"nowrap", flexShrink:0 }}>{it.amount.toLocaleString()}원</span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:11.5, color:C.muted }}>
                    <Calendar size={10} style={{ verticalAlign:-1.5, flexShrink:0 }}/><span>{fmtDate(it.date)}</span><span>·</span><span>{it.label}</span>
                  </div>
                </div>
              );
              return (
                <div key={it.b.id+it.label+idx} onClick={()=>setSelectedBooking(it.b)}
                  ...
                  {/* 고객사 · 모델 */}
                  <span style={{ fontSize:13, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", minWidth:0 }}><strong style={{ fontWeight:700, color:C.text }}>{client?.name||"?"}</strong><span style={{ color:C.muted, fontWeight:600 }}> · {model?.name||"?"}</span></span>
                  {/* 날짜 (별도 셀 · 세로 일렬) */}
                  <span style={{ fontSize:12, color:C.muted, whiteSpace:"nowrap" }}><Calendar size={10} style={{ verticalAlign:-1.5, flexShrink:0 }}/> {fmtDate(it.date)}</span>
                  {/* 계약금/잔금 (별도 셀 · 세로 일렬) */}
                  <span style={{ fontSize:12, color:C.muted, whiteSpace:"nowrap", textAlign:"center", minWidth:40 }}>{it.label}</span>
                  {/* 금액 (맨 오른쪽) */}
                  <span style={{ fontSize:13, fontWeight:800, color:it.label==="잔금"?C.text:C.textSub, whiteSpace:"nowrap", textAlign:"right" }}>{it.amount.toLocaleString()}원</span>
                </div>
              );
            }); })()}
          </div>
    ...
    {(()=>{
      ...
      return (
        <div style={{ position:"relative", background:`linear-gradient(135deg, ${PINK}26 0%, ${PINK}0d 55%, transparent 100%)`, border:`1.5px solid ${PINK}88`, borderRadius:14, padding:"18px 18px 16px", marginBottom:16, boxShadow:`0 0 0 1px ${PINK}22, 0 10px 30px -12px ${PINK}88`, overflow:"hidden" }}>
          ...
          {/* 리스트 — 모바일: 예전 핑크 카드 복원 / 데스크탑: 섭외 리스트 공용 컴포넌트 */}
          {isMobile ? (
            <div style={{ display:"grid", gap:8, gridTemplateColumns:"minmax(0,1fr)" }}>
              {sorted.map(b=>{
                ...
                return (
                  <div key={b.id} onClick={()=>setSelectedBooking(b)} style={{ background:C.card, border:`1px solid ${isToday?PINK+"99":C.border}`, borderRadius:10, padding:"11px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:10, minWidth:0 }}>
            ...
            <BookingsList bookings={sorted} models={models} customers={customers} isMobile={isMobile} onSelect={setSelectedBooking} showHeader={false} />
          )}
        </div>
      );
    })()}
    ...
    <div>
      ...
      {activeBookings.length===0 ? <p style={{ color:C.muted, fontSize:13 }}>진행중인 섭외가 없습니다.</p>
       : isMobile ? (()=>{
        // 모바일: 예전 레이아웃 복원(프로젝트 그룹 카드 + 단건 카드). 데스크탑은 공용 BookingsList.
        const projGroup: Record<string, any[]> = {}; const singles: any[] = [];
        const _now = new Date();
        const todayS = `${_now.getFullYear()}-${String(_now.getMonth()+1).padStart(2,"0")}-${String(_now.getDate()).padStart(2,"0")}`;
        const isOverdue = (d:string) => !!d && d < todayS;
        activeBookings.forEach(b=>{ if(b.project_id){ (projGroup[b.project_id]=projGroup[b.project_id]||[]).push(b); } else singles.push(b); });
       ...
       : <BookingsList bookings={activeBookings} models={models} customers={customers} isMobile={isMobile} onSelect={setSelectedBooking} emptyText="진행중인 섭외가 없습니다." />}
    </div>
  </div>
  );
}
