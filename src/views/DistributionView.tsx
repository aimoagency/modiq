      ...
      <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        {travel && (travel.entry_date || travel.exit_date) && (
          <p style={{ margin: "8px 0 0", fontSize: 11, color: C.blue, fontWeight: 600 }}>
            <Plane size={11} style={{ verticalAlign: -2 }} />
            {travel.entry_date ? ` 입국 ${fmtDate(travel.entry_date)}` : ""}
            {travel.entry_date && travel.exit_date ? " ~" : ""}
        ...
        {onImport && (
          <div style={{ marginTop: "auto", paddingTop: 10 }}>
            {imported
              ? <div style={{ fontSize: 12, fontWeight: 700, color: C.green, textAlign: "center", background: C.green + "14", borderRadius: 8, padding: "8px 0" }}><CheckCircle2 size={13} style={{ verticalAlign: -2 }} /> 내 모델로 등록됨</div>
              : <button onClick={onImport} disabled={importBusy} style={{ ...btnS(C.blue, importBusy), width: "100%" }}>{importBusy ? "등록 중…" : "내 모델로 등록"}</button>}
          </div>
        )}
      </div>
    </div>
  ...
  const Row = (d: TalentDistribution, bt: string) => {
    ...
    return (
      <div key={d.id}
        ...
        style={{ display: "grid", gridTemplateColumns: "minmax(0,320px) auto auto 1fr max-content", alignItems: "center", gap: 14, padding: "11px 16px", borderTop: bt, transition: "background 0.12s", opacity: dim }}>
        <span style={{ display: "flex", alignItems: "baseline", gap: 8, minWidth: 0 }}>
          <strong title={d.message || undefined} style={{ fontSize: 12.5, fontWeight: 700, color: C.text, whiteSpace: "nowrap", flexShrink: 0 }}>모델 {(d.distribution_models || []).length}명 · 파트너 {recs.length}곳</strong>
        ...
        <span />
        <span style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, whiteSpace: "nowrap", paddingRight: 16 }}>{statusBadge}{revokeBtn}</span>
      </div>
    );
  };
