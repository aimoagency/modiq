...
export default function ModelsView({ filteredModels, modelQ, setModelQ, setShowModelForm, setSelectedModel, setMEditMode, bookings, isMobile = false, onBulkAdd }: {
          ...
          {filteredModels.map(m=>{
            ...
            if (isMobile) return (
              <div key={m.id} onClick={()=>{ setSelectedModel(m); setMEditMode(false); }} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 14px", cursor:"pointer" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:5 }}>
                  ...
                  {m.category&&<span style={{ background:C.card2, color:C.textSub, fontSize:10, padding:"2px 7px", borderRadius:10, whiteSpace:"nowrap", flexShrink:0, marginLeft:"auto" }}>{m.category}{age!==null?` · ${age}세`:""}</span>}
                </div>
            ...
            return (
              ...
              >
                ...
                {/* 카테고리/성별 */}
                {m.category&&<span style={{ background:C.card2, color:C.textSub, fontSize:11, padding:"2px 8px", borderRadius:10, whiteSpace:"nowrap" }}>{m.category}{age!==null?` · ${age}세`:""}</span>}
                {/* 외국인 D-day */}
