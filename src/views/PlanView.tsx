  ...
  return (
    <div>
      <div style={{ textAlign:"center", marginBottom:32 }}>
        <h1 style={{ margin:"0 0 8px", fontSize:31, fontWeight:900, color:C.text }}>요금제 선택</h1>
        <p style={{ margin:"0 0 20px", fontSize:15, color:C.muted }}>에이전시 규모에 맞는 요금제를 선택하세요</p>
        <div style={{ display:"inline-flex", background:"var(--c-card2)", borderRadius:8, padding:4, gap:4 }}>
          {["monthly","yearly"].map(t=>(
            <button key={t} onClick={()=>setPlanBilling(t as "monthly"|"yearly")} style={{ padding:"7px 20px", border:"none", borderRadius:6, cursor:"pointer", fontWeight:600, fontSize:14, transition:"all 0.2s", background:planBilling===t?"white":"transparent", color:planBilling===t?"#111":C.muted }}>
              {t==="monthly"?"월간":"연간"}
            </button>
          ))}
        </div>
        {planBilling==="yearly"&&<p style={{ margin:"10px 0 0", fontSize:14, color:C.green, fontWeight:600 }}><PartyPopper size={12} style={{ verticalAlign:-2, flexShrink:0 }}/> 연간 결제 시 최대 28% 할인</p>}
      </div>
          ...
          return (
            ...
            >
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                <h3 style={{ margin:0, fontSize:22, fontWeight:800, color:C.text }}>{plan.name}</h3>
                <div style={{ display:"flex", gap:6 }}>
                  {plan.popular&&!isCurrent&&<span style={{ background:plan.color, color:"white", fontSize:12, fontWeight:800, padding:"4px 10px", borderRadius:20 }}>POPULAR</span>}
                  {isCurrent&&<span style={{ background:plan.color+"22", color:plan.color, border:`1px solid ${plan.color}`, fontSize:12, fontWeight:700, padding:"4px 10px", borderRadius:20 }}>현재 플랜</span>}
                </div>
              </div>
              <div style={{ marginBottom:20 }}>
                <div style={{ display:"flex", alignItems:"baseline", gap:4 }}>
                  <span style={{ fontSize:44, fontWeight:900, color:C.text, lineHeight:1 }}>{(price/10000).toFixed(0)}만</span>
                  <span style={{ fontSize:18, color:C.muted }}>원</span>
                </div>
                <p style={{ margin:"4px 0 0", fontSize:13, color:C.muted }}>{planBilling==="yearly"?"월 환산 · 연간 결제":"매월 청구"}</p>
              </div>
              {myRole==="owner"&&(
                <button onClick={()=>handleChangePlan(plan.id)} disabled={isCurrent} style={{ width:"100%", padding:"12px 0", border:"none", borderRadius:8, cursor:isCurrent?"not-allowed":"pointer", fontWeight:700, fontSize:15, marginBottom:22, transition:"opacity 0.2s", background:isCurrent?"#2a2d3e":plan.color, color:isCurrent?C.muted:"white", opacity:isCurrent?0.7:1 }}>
                  {isCurrent?"현재 요금제":"시작하기"}
                </button>
              )}
              <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:18 }}>
                <p style={{ margin:"0 0 12px", fontSize:13, fontWeight:700, color:C.textSub }}>포함 기능:</p>
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="8" fill={plan.color+"22"}/><path d="M5 8l2 2 4-4" stroke={plan.color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    <span style={{ fontSize:14, color:C.textSub }}>추가 담당자 {cfg.additionalPrice.toLocaleString()}원/명</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <p style={{ textAlign:"center", marginTop:28, fontSize:13, color:C.muted }}>모든 요금제는 14일 무료 체험 후 결제됩니다 · 언제든지 변경/해지 가능</p>
    </div>
  );
}
